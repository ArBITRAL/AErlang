...
init([Mode]) ->
    ...
    mnesia:create_table(aerl_store,
     			[{attributes,record_info(fields,aerl_store)},{index,[#aerl_store.id,#aerl_store.body,#aerl_store.wealth,#aerl_store.eyes,#aerl_store.hair]}]),
    mnesia:create_table(aerl_time,
     			[{attributes,record_info(fields,aerl_time)}]),
...
handle_call({unregister, Pid}, _From, State) ->
    ...
    Reply = case mnesia:dirty_read(aerl_store, Pid) of
		[_Process] ->
		    ...
		    mnesia:dirty_delete(aerl_time, Pid),
		    ok;
		_ -> {error, undefined}
	    end,
    {reply, Reply, State};
handle_call({update_att, Env}, {Pid, _} = _From, State) ->
    %%Rec = ms_util:make_rec(aerl_store,Env),
    Reply = case mnesia:dirty_read(aerl_store, Pid) of
...
register_handle(Pid, Env, From) when is_list(Env) ->
    NewEnv = [{pid,Pid}|Env],
    Rec = ms_util:make_rec(aerl_store,NewEnv),
    ...
    Reply = case mnesia:dirty_read(aerl_store, Pid) of
		...
		_ ->
		    ...
		    mnesia:dirty_write(#aerl_time{pid=Pid,timestamp=Now}),
		    %%io:format("~p~n",[Pid]),
		    %%io:format("~p~n",[Now]),
		    link(Pid),
		    ok
...
predicate_handle(Pred,Pid) ->
    ...
    case mnesia:dirty_read(aerl_store, Pid) of
    	[Process] ->
	    mnesia:dirty_write(Process#aerl_time{pred = Fun});
    	_ -> ok
    end.
