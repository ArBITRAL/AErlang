	...
	true ->
		...
		3 ->
			...
			{match,Line,
			 ...
			 [LocalEval,{'receive',L1,with_insert1(C0,L1)},RecursiveCall]}]}},
		...
		5 ->
			...
			{match,Line,
			 ...
			 [Basecase,
			 {clause, L1, [{var,L1,Num}], [],
			 [LocalEval,{'receive',L1,with_insert1(C0,L1),To,exprs(ToCs)},RecursiveCall]}]}},
		    NewCall = {call,L1,Fname,[CountExp]},
...
%% THis is for receiving pred in the form of string
from_syntax({call, Line, {atom, _, from}, [{string,Line,RawPred} = Pred]}, ReceiveClause) ->
    %% with must be followed by receive clause
     case element(1,ReceiveClause) == 'receive' of
	false -> %transform error
	     throw({"A 'from' construct must be followed by receive",Line});
...
%% This is for receiving predicate in form of var
from_syntax({call, Line, {atom, _, from}, [{var,Line,_RawPred}=Pred]}, ReceiveClause) ->
    %% with must be followed by receive clause
     case element(1,ReceiveClause) == 'receive' of
	false -> %transform error
	     throw({"A 'from' construct must be followed by receive",Line});
	true ->
	     ...
	     %% Create Bindings list in hardway, it is indeed empty binding for this type of predicate
	     VarsNames = [],
