...
-define(WFileOut,"women_prefs.list").
-define(MFileOut,"men_prefs.list").
...
start(Path) ->
    ...
    write_terms(Path++"/"++?MFileOut, MList),
    WList = build_men(WPrefs,NewList1),
    write_terms(Path++"/"++?WFileOut, WList).
...
build_man(Prefs,WAtts,Acc,N) ->
    ...
    %% 				 end
    %% 			 end, Acc, R),
    %% build_man(Prefs,WAtts, lists:reverse(NewAcc) ,N-1).
    NewAcc = lists:foldl(fun(L,Sum) ->
				 Sum ++ [L]
			 end, Acc, R),
