...
from_syntax({call, Line, {atom, _, from}, [Prcv]}, ReceiveClause) ->
	...
	true ->
		...
		3 ->
		    ...
		    Esend = list_to_atom(lists:append("_Envs",integer_to_list(Line))),
		     ...
		     %%%
		    Some = {call,Line,{atom,Line,getName},[{integer,Line,Line}]},
		     NewReceive = {match,Line,
				   ...
				   {'fun',Line,
				    {clauses,
					 ...
					 [{clause,Line,
					   [{tuple,Line,
					     [{var,Line,Psend},{tuple,Line,VarsNames},{var,Line,Bsend},{var,Line,Esend}]}],
					   [],
					   [{'case',Line,
					     {op,Line,'andalso',
					      {call,Line,
					       {remote,Line,{atom,Line,aerl},{atom,Line,check_spred2}},
					       [{var,Line,Psend},{var,Line,'Env'},{var,Line,Bsend}]},
					      {call,Line,
					       {remote,Line,{atom,Line,aerl},{atom,Line,check_rpred2}},
					       [Prcv,VarsBinding,{var,Line,Esend}]}},
					     [{clause,Line,
					       [{atom,Line,true}],
					       [],
					       [{op,Line,'!',
						 {call,Line,{atom,Line,self},[]},
						 {tuple,Line,VarsNames}},
...
with_insert([{clause,Line,Vars,Arr,List}]) ->
...
expr({op,Line,'!',{call, Line, {atom, Line, to},[{var,Line,Pred}]},{tuple,Line,[Content1,Content2]}}) ->
  {call,Line,
      {remote,Line,{atom,Line,aerl},{atom,Line,send}},
       [{var,Line,Pred},Content1,Content2]};
expr({op,Line,'!',{call, Line, {atom, Line, to},[{string,Line,Pred}]},{tuple,Line,[Content1,Content2]}}) ->
  Rx = "[\\$|\\?](\\w+)",
  VarsNames = case re:run(Pred, Rx, [global, {capture, all_but_first, list}]) of
        {match, Values} -> lists:concat(Values);
        _Other -> []
