...
from_syntax({call, Line, {atom, _, from}, [PredExp,CountExp]}, ReceiveClause) ->
	...
	true ->
	     ...
	     LocalEval = %{match,Line,Predname,
			  {call,Line,
	     		  {remote,Line,{atom,Line,aerl_check},{atom,Line,local_eval_receive}},
	     		  [PredExp,Bindings]},
		...
		3 ->
			...
			{match,Line,
			 ...
			 [{'receive',L1,with_insert1(C0,L1)},RecursiveCall]}]}},
		...
		5 ->
			...
			{match,Line,
			 ...
			 [Basecase,
			 {clause, L1, [{var,L1,Num}], [],
			 [{'receive',L1,with_insert1(C0,L1),To,exprs(ToCs)},RecursiveCall]}]}},
		    NewCall = {call,L1,Fname,[CountExp]},
		    [LocalEval,NewReceive,NewCall]
            end
     end;

from_syntax({call, Line, {atom, _, from}, [Pred]}, ReceiveClause) ->
    %% with must be followed by receive clause
     case element(1,ReceiveClause) == 'receive' of
	false -> %transform error
	     throw({"A 'from' construct must be followed by receive",Line});
	true ->
	     ...
	     Predname = {var,Line,list_to_atom(lists:append("_Rpred",integer_to_list(Line)))},
	     %% Function later
	     %% NewPred = case stringify_fun(Pred) of
	     %% 		   [] -> Pred;
	     %% 		   Func -> Func
	     %% 	       end,
	     {_,Line,RawPred} = Pred,
		...
		3 ->
		    {'receive',L1,C0} = ReceiveClause,
		    NewReceive =
			{'receive',L1,with_insert(C0,Fooname)},
		    %% case stringify(_L,RawPred) of
		    %% 	[] -> [Localupdate,NewReceive];
		    %% 	_ -> [LocalEval,Localupdate,NewReceive]
		    %% end;
		...
		5 ->
		    ...
		    %% Fooname = {var,Line,list_to_atom(lists:append("_Foo",integer_to_list(Line)))},
		    %%{'receive',L1,C0,To,ToCs} = ReceiveClause,
		    %% Converted = case Pred of
		    %% 		    {var, _, _} -> Pred;
		    %% 		    {string, _, Raw} ->
		    %% 			{string, Line, re:replace(Raw, "[$]", "this.", [global, {return, list}])}
		    %% 		end,
		    %% NewReceive =
		    %% 	{match,Line,
		    %% 	 Fname,
		    ...
		    %% 	     [Fooname],
		    %% 	     [],
		    %% 	     [{'receive',L1,with_insert(C0,Fooname),To,exprs(ToCs)}]
		    %% 	    }]}}},

		    %% NewCall =  {call,Line,Fname,[Fname]},
		    %% {_,_,RawPred} = Pred,
		    %% case stringify(Line,RawPred) of
		    %% 	[] -> [LocalEval,NewReceive,NewCall];
		    %% 	[C1,C3] -> [C1,LocalEval,NewReceive,NewCall,C3]
		    %% end
		    %% {'receive',L1,C0,To,ToCs} = ReceiveClause,
		    %% {integer,L2,_} = To,
		    %% Converted = re:replace(Pred, "[$]", "this.", [global, {return, list}]),
		    %% BrokerReceive = {call,Line,
		    %% 		  {remote,Line,{atom,Line,aerl_broker},{atom,Line,a_receive}},
		    %% 		  [{string,Line,Converted},To]},
		    %% NewReceive = {'receive',L1,with_insert(C0),{integer,L2,0},exprs(ToCs)},
		    %% case stringify(Line,Pred) of
		    %% 	[] -> [BrokerReceive,NewReceive];
		    %% 	[C1,C3] -> [C1,BrokerReceive,C3,NewReceive]
		    %% end
            end
     end.
...
with_modify({clause,Line,[Vars],Arr,List},Fooname) ->
    Spred = {var,Line,list_to_atom(lists:append("_Spred",integer_to_list(Line)))},
    Senv = {var,Line,list_to_atom(lists:append("_Senv",integer_to_list(Line)))},
    NewVars = {tuple,Line,[Vars,Spred,Senv]},
    %% NewList =
    %% 	[{'case',Line,
    %% 	   BrokerCheck,
    %% 	  [{clause,Line,
...
expr({op,Line,'!',{call, Line, {atom, Line, to_c},[{Type,Line,Pred}]},Content}) ->
