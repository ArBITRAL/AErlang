...
exprs([E0|Es]) ->
    case lists:any(fun ({call,_Line,{atom,_Line,with},_}) -> true; (_) -> false end,[E0]) of
	false ->
            E1 = expr(E0),
	    [E1|exprs(Es)];
	true ->
...
%% with construct by
%% expr({call, Line, {atom, _Line1, with},
%% handle this contruct with the receive construct at once
with_syntax({call, Line, {atom, _, with}, [{_, _, Var}]}, ReceiveClause) ->
    %% with must be followed by receive clause
    case element(1,ReceiveClause) == 'receive' of
	false -> throw({"A 'with' construct must be followed by receive",Line});
	true ->
...
with_helper({clause,Line,[{tuple,_,From}]=Msg,Arr,Exp}=C,Var) ->
    case lists:any(fun( {var,_,_} ) -> true; (_) -> false end, [lists:last(From)]) of
	false -> C;
	true ->
	    {var,_,EPid} = lists:last(From),
	    {clause,Line,Msg,Arr,with_modify(Exp,Var,EPid)}
    end;
with_helper({clause,Line,[{var,_,_}]=Msg,Arr,Exp},Var) ->
    {var,_,EPid} = lists:last(Msg),
    {clause,Line,Msg,Arr,with_modify(Exp,Var,EPid)};
with_helper(C0,_Var) ->
    %% Other cases?
    C0.
